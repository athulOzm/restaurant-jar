...
<a href="{{route('pos.clone', $order->id)}}" class="btn btn-info"> <i class="fas fa-clone"></i> Copy</a>
            ...
            <a onclick="deleteCon('delfrmad{{$order->id}}');" class="btn btn-sm btn-danger "><i class="fas fa-trash"></i> Cancel</a>
            
            <form id="delfrmad{{$order->id}}" action="{{route('order.destroy')}}" method="post">
                @csrf
                @method('DELETE')
