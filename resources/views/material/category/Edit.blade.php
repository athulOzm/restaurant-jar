<?php
$categories = resolve('allPcategories');
?>
                    ...
                    <div class="card shadow mb-12" style="width:100%">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                ...
                                <form class="w-full px-6 space-y-6 sm:px-10 sm:space-y-8" method="POST" action="{{ route('pcategory.update') }}"  enctype='multipart/form-data'>
                                    @csrf
                                    @method('PATCH')
                                    <input type="hidden" value="{{$pcategory->id}}" name="id">
                            <input type="hidden" value="{{$pcategory->cover}}" name="curimage">
                
                                    <div class="form-group">
                                        <label for="name" class="block text-gray-700 text-sm font-bold mb-2 sm:mb-4 ">
                                            Name:
                                        </label>
                                        <input id="name" type="name" value="{{$pcategory->name}}"
                                    ...
                                    <div class="form-group">
                                        <label for="parant" class="block text-gray-700 text-sm font-bold mb-2 sm:mb-4 ">
                                            ...
                                            <select class="form-control w-full border-gray-400" name="parant">
                                                ...
                                                @if ($pcategory->parant()->exists())
                                                    <option value="{{$pcategory->parant->id}}">{{$pcategory->parant->name}}</option>   
                                                @else
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input id="order" type="number" value="{{$pcategory->order}}"
                                ...
                                <div class="row">
                                    <div class="col-md-9">
                                        @if ($pcategory->cover != null)
                                    <img class="img-thumbnail " src="{{env('IMAGE_PATH')}}{{ $pcategory->cover}}" />
                                    @endif
                                    </div>
                                </div>
                                    </div>
